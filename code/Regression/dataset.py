...
class Graph_Bert_Dataset_fine_tune(object):
    def __init__(self,tst_set,tokenizer,batch_size,label_field='DDI'):
    ...
    def get_data(self):
        ...
        self.dataset1 = tf.data.Dataset.from_tensor_slices((self.tst_set['drug_A'], self.tst_set['drug_B'],self.tst_set[self.label_field]))
    ...
    def numerical_seq(self,smiles1,smiles2,labels):
        ...
        ### Process molecule 1
        nums_list1, edge1,clique1= self.tokenizer.tokenize(smiles1)
        adjoin_matrix1 = get_adj_matrix(nums_list1,edge1)
        dist_matrix1 = get_dist_matrix(nums_list1, edge1)
        ...
        ### Process molecule 2
        nums_list2, edge2,clique2= self.tokenizer.tokenize(smiles2)
        adjoin_matrix2 = get_adj_matrix(nums_list2,edge2)
        dist_matrix2 = get_dist_matrix(nums_list2, edge2)
...
class Inference_dataset(object):
    def __init__(self,tst_set,tokenizer,batch_size):
        self.tst_set = tst_set
        self.batch_size = batch_size
        self.tokenizer = tokenizer
        self.pad_value = self.tokenizer.vocab['<pad>'] 
    ...
    def get_data(self):
        ...
        self.dataset1 = tf.data.Dataset.from_tensor_slices((self.tst_set['drug_A'], self.tst_set['drug_B']))
        self.dataset1 = self.dataset1.map(self.tf_numerical_seq).padded_batch(self.batch_size, padding_values= (tf.constant(0,dtype = tf.int64),tf.constant(0,dtype = tf.int64),\
            tf.constant(0,dtype = tf.float32),tf.constant(0,dtype = tf.float32),tf.constant(-1e9,dtype = tf.float32),\
                tf.constant(-1e9,dtype = tf.float32),tf.constant(0,dtype = tf.float32),tf.constant(0,dtype = tf.float32),\
                    tf.constant(0,dtype = tf.float32),tf.constant(0,dtype = tf.float32),tf.constant(-1e9,dtype = tf.float32),\
        ...
        padded_shapes=(
            tf.TensorShape([None]),tf.TensorShape([None]),tf.TensorShape([None,None]),tf.TensorShape([None,None]),\
                tf.TensorShape([None,None]),tf.TensorShape([None,None]),tf.TensorShape([None,None]),tf.TensorShape([None,None]),\
                tf.TensorShape([None,None]),tf.TensorShape([None,None]),tf.TensorShape([None,None]),tf.TensorShape([None,None]),\
                tf.TensorShape([None,None]),tf.TensorShape([None,None]),tf.TensorShape([None,None]),tf.TensorShape([None,None]))).prefetch(20)
        return self.dataset1

...
class Graph_Bert_Dataset_fine_tune(object):
    def __init__(self,train_set,val_set,tst_set,tokenizer,batch_size,label_field='DDI'):
        
        self.train_set  = train_set
        self.val_set = val_set
        self.tst_set = tst_set
        self.train_set[label_field] = self.train_set[label_field].map(float)
        self.val_set[label_field] = self.val_set[label_field].map(float)
        self.tst_set[label_field] = self.tst_set[label_field].map(float)
        self.label_field = label_field
        self.batch_size = batch_size
        self.tokenizer = tokenizer
        # self.window_size = window_size
        self.pad_value = self.tokenizer.vocab['<pad>']
        self.map_dict = np.load('/code/Regression/preprocessed_drug_info_for_regression.npy',allow_pickle=True).item()
        ...
        self.dataset1 = self.dataset1.map(self.tf_numerical_seq).padded_batch(self.batch_size, padding_values= (tf.constant(0,dtype = tf.int64),tf.constant(0,dtype = tf.int64),\
            tf.constant(0,dtype = tf.float32),tf.constant(0,dtype = tf.float32),tf.constant(-1e9,dtype = tf.float32),\
                tf.constant(-1e9,dtype = tf.float32),tf.constant(0,dtype = tf.float32),tf.constant(0,dtype = tf.float32),\
                    tf.constant(0,dtype = tf.float32),tf.constant(0,dtype = tf.float32),tf.constant(-1e9,dtype = tf.float32),\
                tf.constant(-1e9,dtype = tf.float32),tf.constant(0,dtype = tf.float32),tf.constant(0,dtype = tf.float32),\
                    tf.constant(1,dtype = tf.float32),tf.constant(1,dtype = tf.float32),tf.constant(0,dtype = tf.float32)),
        padded_shapes=(
...
class Inference_Dataset(object):
    def __init__(self,drug_pair_dict,tokenizer):
        ...
        self.tokenizer = tokenizer
    def get_data(self): #Train:val:tst = 8:1:1 

        self.dataset = tf.data.Dataset.from_tensor_slices((self.drug_A,self.drug_B))
        self.dataset = self.dataset.map(self.tf_numerical_smiles).padded_batch(1, padding_values= (tf.constant(0,dtype = tf.int64),tf.constant(0,dtype = tf.int64),\
            tf.constant(0,dtype = tf.float32),tf.constant(0,dtype = tf.float32),tf.constant(-1e9,dtype = tf.float32),\
        ...
        padded_shapes=(tf.TensorShape([None]),tf.TensorShape([None]),
             ...
             tf.TensorShape([None, None]), tf.TensorShape([None, None]))).cache()#.prefetch(20)

        return self.dataset
        
    def numerical_smiles(self,smiles1,smiles2):
        smiles1 = smiles1.numpy().decode()
        smiles2 = smiles2.numpy().decode()
        # seq = '[GLOBAL]' + seq
        # tokens = self.tokenizer.encode(seq)
        # nums_list = tokens.ids
        # windows_matrix = self.generate_window_matrix(nums_list,self.window_size)
        nums_list1, edges1,_= self.tokenizer.tokenize(smiles1)
        ...
        temp1_dist[1:,1:] = dist_matrix1
        dist_matrix1 = temp1_dist
        nums_list2, edges2,_= self.tokenizer.tokenize(smiles2)
    ...
    def tf_numerical_smiles(self, smiles1,smiles2):
        
        x1,x2,adjoin_matrix1,adjoin_matrix2,dist_matrix1,dist_matrix2 = tf.py_function(self.numerical_smiles,
                                        [smiles1,smiles2], [tf.int64, tf.int64,tf.float32 ,tf.float32,tf.float32 ,tf.float32])

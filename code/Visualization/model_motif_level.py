...
class EncoderModel_motif(keras.layers.Layer):
    def __init__(self, num_layers, input_vocab_size,
        ...
        # self.activate_dist_matrix = keras.layers.Dense(100)
        # self.dist_matrix_heads = [keras.layers.Dense(100) for _ in range(num_heads)]
        # self.learnable_v = [keras.layers.Dense(100) for _ in range(num_heads)]
        # self.variables_w = [tf.Variable(0.5,trainable=True,dtype=tf.float32,name= 'w_{}'.format(i)) for i in range(num_heads)]
        # self.variables_v = [tf.Variable(0.5,trainable=True,dtype=tf.float32,name= 'v_{}'.format(i)) for i in range(num_heads)] 
...
class Co_Attention_Layer(keras.layers.Layer):
    ...
    def call(self, inputs): 
        ...
        H_q = tf.multiply(tf.tanh(tf.matmul(self.W_q,Q_r)),tf.tanh(tf.matmul(self.W_m,M_0)))#上下权重相同
        ...
        return tf.squeeze(vector_v,1), tf.squeeze(vector_q,1),alpha_v,alpha_q#,inputs[2]
